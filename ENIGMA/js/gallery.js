...
document.addEventListener('DOMContentLoaded', function() {
    // Images and Videos from assets/enigma xii/ folder (excluding sponsor images)
    const galleryItems = [
    ...
    if (galleryItems.length > 0 && galleryGrid) {
        galleryItems.forEach((item, index) => {
            ...
            if (item.type === 'video') {
                ...
                video.src = `assets/enigma xii/${item.src}`;
                if (item.poster) {
                    video.poster = `assets/enigma xii/${item.poster}`;
                }
                video.muted = true;
                ...
                img.src = `assets/enigma xii/${item.src}`;
